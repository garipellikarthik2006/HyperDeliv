...
const DEMO_ADMIN = { id: 100, email: "admin@demo.com", password: "admin123", fullName: "Admin User", role: "admin" };
...
const AdminLogin = () => {
  ...
  const handleGoogleSuccess = (googleUser) => {
    const authUser = { id: Date.now(), email: googleUser.email, role: "admin", fullName: googleUser.name, picture: googleUser.picture };
  ...
  const onSubmit = async (e) => {
    ...
    try {
      if (email === DEMO_ADMIN.email && password === DEMO_ADMIN.password) {
        const authUser = { id: DEMO_ADMIN.id, email: DEMO_ADMIN.email, role: DEMO_ADMIN.role, fullName: DEMO_ADMIN.fullName };
        const fakeToken = btoa(JSON.stringify(authUser));
        localStorage.setItem("dabbaToken", fakeToken);
        localStorage.setItem("dabbaUser", JSON.stringify(authUser));
        localStorage.setItem("isLoggedIn", "true");
      ...
      toast({ title: "Login Failed", description: "Invalid credentials. Use admin@demo.com / admin123", variant: "destructive" });
    } catch {
