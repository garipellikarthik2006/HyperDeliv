...
const UserLogin = () => {
  ...
  useEffect(() => {
    if (googleButtonRef.current) {
      initializeGoogleButton(googleButtonRef.current);
    }
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (existingUsers.length === 0) localStorage.setItem("dabbaflow_users", JSON.stringify(DEMO_USERS));
      const storedUsers = JSON.parse(localStorage.getItem("dabbaflow_users") || "[]");
      if (!storedUsers.some((u) => u.email === "user@demo.com")) {
        localStorage.setItem("dabbaflow_users", JSON.stringify([...storedUsers, ...DEMO_USERS]));
      }
      const finalUsers = JSON.parse(localStorage.getItem("dabbaflow_users") || "[]");
      const foundUser = finalUsers.find((u) => u.email === email && u.password === password);
      if (foundUser) {
        const authUser = { id: foundUser.id ?? Date.now(), email: foundUser.email, role: foundUser.role, fullName: foundUser.fullName };
        const fakeToken = btoa(JSON.stringify(authUser));
        localStorage.setItem("dabbaToken", fakeToken);
        localStorage.setItem("dabbaUser", JSON.stringify(authUser));
        localStorage.setItem("isLoggedIn", "true");
        localStorage.setItem("userRole", authUser.role);
        toast({ title: "Welcome back!", description: "Successfully logged in as " + foundUser.fullName });
        window.location.href = "/dashboard";
        return;
      }
      toast({ title: "Login Failed", description: "Invalid email or password. Use user@demo.com / user123", variant: "destructive" });
    } catch {
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8 transition-shadow duration-300 hover:shadow-2xl">
          ...
          <div className="mt-6 text-center">
            <p className="text-sm text-gray-600">
              New Customer?{" "}
              <button onClick={() => navigate("/signup/user")} className="text-orange-500 hover:text-orange-600 hover:underline font-medium">
                Start here
              </button>
            </p>
          </div>
